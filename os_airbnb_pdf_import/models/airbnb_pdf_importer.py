...
class AirbnbPdfImporter(models.TransientModel):
    ...
    def _create_partner(self, data):
        ...
        # Préparer les valeurs pour la recherche et la création
        firstname = data.get('first_name') or 'Client'
        lastname = data.get('last_name') or 'Airbnb'
        phone = data.get('phone')
        phone_raw = data.get('phone_raw', '')

        # 1. Recherche par téléphone (prioritaire si disponible)
        partner = None
        if phone:
            ...
            if partner:
                _logger.info(f"Partner trouvé par téléphone: {partner.name} (ID: {partner.id})")
                # Optionnel : Mettre à jour les informations si elles ont changé
                self._update_partner_if_needed(partner, data)
                return partner

        # 2. Recherche par nom complet si pas trouvé par téléphone
        if data.get('first_name') and data.get('last_name'):
            ...
            if partner:
                _logger.info(f"Partner trouvé par nom: {partner.name} (ID: {partner.id})")
                # Optionnel : Mettre à jour les informations si elles ont changé
                self._update_partner_if_needed(partner, data)
                return partner

        # 3. Recherche par URL Airbnb (si disponible)
        if data.get('website'):
            partner = Partner.search([
            ...
            if partner:
                _logger.info(f"Partner trouvé par URL Airbnb: {partner.name} (ID: {partner.id})")
                self._update_partner_if_needed(partner, data)
                return partner

        # 4. Aucun partner trouvé, en créer un nouveau
        _logger.info(f"Création d'un nouveau partner: {firstname} {lastname}")
        ...
        partner_vals = {
            ...
            'is_company': False,
            'customer_rank': 1,
            'category_id': [(6, 0, self.env.ref('os_airbnb_pdf_import.res_partner_category_plateforme_airbnb').ids)]
        }

        partner = Partner.create(partner_vals)
        _logger.info(f"Nouveau partner créé: {partner.name} (ID: {partner.id})")

        return partner

    def _update_partner_if_needed(self, partner, data):
        """
        Met à jour les informations du partner si nécessaire
        (photo, téléphone, ville, etc.)
        """
        update_vals = {}

        # Mettre à jour la photo si elle n'existe pas ou si une nouvelle est fournie
        if data.get('image_1920') and not partner.image_1920:
            update_vals['image_1920'] = data['image_1920']

        # Mettre à jour le téléphone si vide
        if data.get('phone_raw') and not partner.mobile:
            update_vals['mobile'] = data['phone_raw']

        # Mettre à jour l'URL Airbnb si vide
        if data.get('website') and not partner.website:
            update_vals['website'] = data['website']

        # Mettre à jour la ville si vide
        if data.get('city') and not partner.city:
            update_vals['city'] = data['city']

        # Mettre à jour le pays si vide
        ...
        # Ajouter la catégorie Airbnb si elle n'existe pas
        airbnb_category = self.env.ref('os_airbnb_pdf_import.res_partner_category_plateforme_airbnb',
                                       raise_if_not_found=False)
        if airbnb_category and airbnb_category not in partner.category_id:
            update_vals['category_id'] = [(4, airbnb_category.id)]

        # Appliquer les mises à jour si nécessaire
        if update_vals:
            partner.write(update_vals)
            _logger.info(f"Partner {partner.name} (ID: {partner.id}) mis à jour avec: {list(update_vals.keys())}")
    ...
    def _add_booking_line_to_booking_month(self, booking_line):
        ...
        Utilise la méthode create_or_update_month pour gérer la vue mensuelle.
        """
        ...
        # Utiliser la méthode existante pour obtenir ou créer le booking_month
        ...
        # Lier la réservation (Odoo gère automatiquement la relation bidirectionnelle)
