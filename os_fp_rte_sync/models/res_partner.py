...
class ResPartner(models.Model):
    ...
    def _get_tahiti_number(self):
        """Récupère le numéro TAHITI depuis res.partner.id_number"""
        self.ensure_one()
        id_number = self.env['res.partner.id_number'].search([
            ('partner_id', '=', self.id),
            ('category_id.code', '=', 'tahiti')
        ], limit=1)
        return id_number.name if id_number else False

    def _get_etablissement_number(self):
        """Récupère le numéro d'établissement RTE depuis res.partner.id_number"""
        self.ensure_one()
        id_number = self.env['res.partner.id_number'].search([
            ('partner_id', '=', self.id),
