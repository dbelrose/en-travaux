...
# Import conditionnel de queue_job
try:
    ...
    _logger.warning("Module queue_job non disponible, utilisation du mode synchrone")


    # Décorateur dummy si queue_job n'est pas disponible
    def job(*args, **kwargs):
        def decorator(func):
            return func
...
class RteSyncWizard(RteSyncMixin, models.TransientModel):
    ...
    def action_sync_now(self):
        """Lance la synchronisation en arrière-plan via cron."""
        force = bool(self.env.context.get("force"))

        # Utiliser directement le cron (plus fiable que queue_job pour les gros imports)
        ...
        return {
            ...
            "params": {
                ...
                "message": "La synchronisation démarre en arrière-plan. Consultez l'historique dans quelques instants.",
                "type": "success",
                "sticky": False,
            },
        }
...
class RteSyncRunner(RteSyncMixin, models.AbstractModel):
    ...
    def schedule_sync_once(self, force=False):
        """Option 'one-shot' via cron (alternative à queue_job)."""
    ...
    @job
    def run_sync_job(self, force=False):
        """Entrée job queue_job."""
        return self.run_sync(force=force)
    ...
    def run_sync(self, force=False):
        ...
        try:
            ...
            # Pré-générer le cache d'images si l'auto-assignation est activée
            if (
                    self.env["ir.config_parameter"]
                            .sudo()
                            .get_param("fp_rte_sync.auto_assign_images", default="True")
                            .lower()
                    in ("1", "true", "yes", "y")
            ):
                _logger.info("Pré-génération du cache d'images...")
            ...
            for row_l, tahiti, is_est, etab_key, naf, is_active_row in rows:
                ...
                if is_est and etab_key and not fold_single:
                    ...
                    # Auto-assign images if enabled (uniquement si pas d'image déjà présente)
                    if (
                            self.env["ir.config_parameter"]
                                    .sudo()
                                    .get_param("fp_rte_sync.auto_assign_images", default="True")
                        ...
                        if hasattr(parent, "image_128") and not parent.image_128:
                            # Utiliser la méthode optimisée avec cache
                            self.env['res.partner']._auto_assign_image(parent)
                        c = by_etab.get(etab_key)
                        if c and hasattr(c, "image_128") and not c.image_128:
                            # Utiliser la méthode optimisée avec cache
                            self.env['res.partner']._auto_assign_image(c)
                    ...
                    # Auto-assign image si pas déjà présente
                    if (
                            self.env["ir.config_parameter"]
                                    .sudo()
                                    .get_param("fp_rte_sync.auto_assign_images", default="True")
                ...
                # Commit par lot uniquement si job/cron
                if count % batch_size == 0 and (self.env.context.get("job_uuid") or self.env.context.get("cron_id")):
                    self.env.cr.commit()
