...
class RteSyncMixin(models.AbstractModel):
    ...
    def _apply_tags(self, partner, naf_code):
...
class RteSyncRunner(RteSyncMixin, models.AbstractModel):
    ...
    def run_sync(self, force=False):
        """
        Synchronisation RTE via l'API tabulaire data.gouv.fr avec fallback sur API classique.

        Gère automatiquement les cas où la ressource est marquée comme supprimée
        sur l'API tabulaire mais reste accessible via l'API classique.
        """
        ...
        try:
            ...
            # Vérifier si une nouvelle version existe (si possible)
            checksum = None
            naf_whitelist = self.env["ir.config_parameter"].sudo().get_param(
                "fp_rte_sync.naf_whitelist", default=""
            ).strip()
            last_naf_filter = self.env["ir.config_parameter"].sudo().get_param("fp_rte_sync.last_naf_filter", "")

            # Le filtre NAF a changé, on doit re-synchroniser même si le checksum est identique
            naf_filter_changed = (naf_whitelist != last_naf_filter)
            ...
            try:
                ...
                if not force and not naf_filter_changed and checksum and last_checksum and checksum == last_checksum:
                    run.write({
                        "status": "skipped",
                        "checksum": checksum,
                ...
                _logger.warning(
                    f"RTE: Impossible de vérifier le checksum via metadata ({e}), synchronisation forcée...")
                checksum = None  # On synchronise sans vérification de checksum

            # -------- Tentative API Tabulaire avec fallback automatique --------
            ...
            try:
                _logger.info("RTE: Tentative de récupération via API tabulaire...")
                ...
                _logger.info("RTE: API tabulaire disponible, récupération des données...")
                all_rows = self._fetch_tabular_data(base_api_url, rid)

            except Exception as e:
                error_msg = str(e)
                _logger.warning(f"RTE: Échec API tabulaire ({error_msg}), basculement sur API classique...")

                # Fallback sur l'API classique (téléchargement CSV)
                use_classic_api = True
                try:
                    csv_url = f"https://www.data.gouv.fr/api/1/datasets/r/{rid}"
                    _logger.info(f"RTE: Téléchargement CSV depuis {csv_url}")

                    csv_content = _http_get(csv_url, timeout=300)
                    csv_text = csv_content.decode('utf-8-sig')  # Support BOM UTF-8

                    # Parser le CSV avec le bon délimiteur (point-virgule)
                    csv_reader = csv.DictReader(io.StringIO(csv_text), delimiter=';')
                    headers = csv_reader.fieldnames or []
                    all_rows = [row for row in csv_reader]

                    _logger.info(f"RTE: {len(all_rows)} lignes récupérées via API classique (CSV)")

                except Exception as csv_error:
                    raise ValueError(
                        f"Échec API tabulaire ET API classique. "
                        f"Tabular: {error_msg} | Classic: {str(csv_error)}"
                    )

            if not headers:
                raise ValueError("Impossible de récupérer les en-têtes de colonnes.")

            if not all_rows:
                raise ValueError("Aucune donnée récupérée.")

            _logger.info(
                f"RTE: {len(all_rows)} lignes totales récupérées "
                f"via {'API classique (CSV)' if use_classic_api else 'API tabulaire'}"
            )
            ...
            _logger.info(f"RTE headers disponibles: {headers_lc[:10]}...")  # Premiers headers pour debug
            ...
            # -------- Filtrage NAF (adapté selon la source) --------
            # naf_whitelist déjà récupéré plus haut pour la vérification du cache

            # -------- 1ère passe : filtrage + collecte clés (dé-dup) ----------
            rows = []
            tahitis_set, etab_keys_set = set(), set()
            uniq_etabs_by_tahiti = {}
            seen_pairs = set()

            _logger.info("RTE: 1ère passe - filtrage et collecte...")
            debug_first_row = True  # Pour debug
            for row_dict in all_rows:
                # Debug première ligne
                if debug_first_row:
                    _logger.info(f"RTE: Exemple première ligne (clés): {list(row_dict.keys())[:10]}...")
                ...
                # Debug NAF pour comprendre le filtrage
                if len(rows) < 5:  # Log des 5 premières lignes valides
                    _logger.info(
                        f"RTE: Ligne {len(rows) + 1} - TAHITI={tahiti}, NAF={naf}, allowed={self._naf_allowed(naf)}")
            ...
            for row_dict, tahiti, is_est, etab_key, naf, is_active_row in rows:
                ...
                street = " ".join(
                    x for x in [(rv(row_dict, "street_num") or "").strip(), (rv(row_dict, "street_name") or "").strip()]
                    if x
                )
                street2 = " - ".join(
                    x for x in [
                ...
                if is_est and etab_key and not fold_single:
                    ...
                    # Étiquette APE uniquement (effectif géré via addon)
                    self._apply_tags(parent, naf)
                    self._apply_tags(child, naf)

                    # Gestion effectif via addon (classe d'effectif numérique prioritaire)
            ...
            # Fin de sync / persistance checksum et filtre NAF
            if checksum:
                self.env["ir.config_parameter"].sudo().set_param("fp_rte_sync.last_checksum", checksum)
            # Sauvegarder le filtre NAF utilisé pour cette synchronisation
            self.env["ir.config_parameter"].sudo().set_param("fp_rte_sync.last_naf_filter", naf_whitelist)

            msg = f"OK – créés:{created}, maj:{updated}, ignorés:{skipped} (via {'CSV' if use_classic_api else 'API tabulaire'})"
            run.write({
                "status": "done",
                "checksum": checksum,
    ...
    def _fetch_tabular_data(self, base_api_url, rid):
        """
        Récupère les données via l'API tabulaire avec pagination.
        Lève une exception si l'API échoue (pour déclencher le fallback).
        """
        page = 1
