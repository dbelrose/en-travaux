...
class BookingImport(models.Model):
    ...
    def action_import_excel_file(self):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                self._add_booking_line_to_booking_month(line)
    ...
    def _create_monthly_views(self):
        ...
        for line in self.line_ids:
            if line.arrival_date:
                year = line.arrival_date.year
                month = line.arrival_date.month
                months_to_create.add((line.year, month))

        for year, month in months_to_create:
            self.env['booking.month'].create_or_update_month(year, month)
    ...
    def _add_booking_line_to_booking_month(self, booking_line):
        ...
        """
        if not booking_line or not booking_line.arrival_date:
            _logger.warning(
                f"Impossible d'ajouter la réservation au mois : "
                f"données manquantes (arrival_date)"
            )
            return
