...
def build_invoice_comment(env, reservation):
    ...
    # Base affichée : privilégier la valeur du modèle si fournie, sinon re-calcul
...
class BookingMonth(models.Model):
    ...
    property_type_id = fields.Many2one(
        ...
        help='Type de propriété concerné par cette vue mensuelle'
    )
    ...
    def _compute_display_name(self):
        for record in self:
            if record.month and record.year and record.property_type_id:
                try:
                    record.display_name = f"{record.property_type_id.name} - {record.month:02d}/{record.year}"
                except (ValueError, AttributeError):
                    record.display_name = f"{record.month:02d}/{record.year}"
    ...
    def _compute_total_commission_booking(self):
        for record in self:
            record.total_commission_booking = \
                sum(r.commission_amount for r in record.reservation_ids if r.commission_amount)
    ...
    def _compute_net_revenue(self):
        for record in self:
            ...
            record.total_costs = \
                record.total_commission_booking + record.total_tourist_tax + record.concierge_commission
            record.profit_margin = record.gross_revenue - record.total_costs
    ...
    _sql_constraints = [
        ('unique_month_property',
         'unique(year, month, company_id, property_type_id)',
         'Une seule vue mensuelle par mois, société et type de propriété!')
    ]
    ...
    # Méthode utilitaire pour forcer le re-calcul
    def _ensure_concierge_partner(self):
        """Force le re-calcul et vérifie la présence du partenaire concierge."""
        self.ensure_one()

        # Force le re-calcul en invalidant le cache
        self.invalidate_recordset(['concierge_partner_id'])

        # Accéder au champ pour déclencher le re-calcul
    ...
    def action_generate_concierge_invoice(self):
        ...
        if not concierge_service:
            raise ValueError(
                """Produit 'Commission conciergerie' introuvable! 
                Veuillez créer un produit avec le code 'COMMISSION_CONCIERGE'. (1)""")
    ...
    def action_generate_booking_invoice(self):
        ...
        for reservation in reservations:
            ...
            if montant > 0:
                ...
                grouped_invoices.setdefault(key, {
        ...
        # Créer les factures
        created_invoices = []
        for key, vals in grouped_invoices.items():
            # Vérifier si la facture existe déjà
    ...
    @api.model
    def create_or_update_month(self, year, month, company_id=None, property_type_id=None):
        """
        ...
        Args:
            ...
            property_type_id (int, required): ID du type de propriété
        """
        ...
        if not property_type_id:
            raise ValueError("Le type de propriété (property_type_id) est obligatoire")
        ...
        if not property_type.exists():
            raise ValueError(f"Type de propriété inexistant: {property_type_id}")
    ...
    def _is_valid_period(self):
        ...
        return self.year and self.month
    ...
    def _get_month_reservations(self):
        """Récupère toutes les réservations du mois pour ce type de propriété"""
        if not self.period_start or not self.period_end or not self.property_type_id:
    ...
    def action_recalculate(self):
        ...
        #     'tag': 'reload',
        # }
        """Force le re-calcul des données avec mise à jour des métadonnées"""

        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': 'Re-calcul terminé',
    ...
    def action_generate_both_concierge_invoices(self):
        ...
        if self.company_id.hm_airbnb_vendor_concierge_commission or \
                self.company_id.hm_booking_vendor_concierge_commission:
            self.action_generate_concierge_invoice()

        # Puis créer la facture client
        if self.company_id.hm_airbnb_customer_concierge_commission or \
                self.company_id.hm_booking_customer_concierge_commission:
    ...
    def action_generate_all_invoices(self):
        """Génère toutes les factures mensuelles (Booking.com globale et concierges par propriété)"""
        self.ensure_one()

        results = []

        # Récupérer tous les enregistrements du mois pour cette société
        monthly_records = self.env['booking.month'].search([
            ('year', '=', self.year),
            ('month', '=', self.month),
            ('company_id', '=', self.company_id.id)
        ...
        # 1. FACTURE BOOKING.COM GLOBALE (une seule pour toute la société)
        try:
            ...
            _logger.info(
                f"Total commission Booking.com pour {self.month}/{self.year}: {total_booking_commission}, Facture existante: {has_booking_invoice}")

            if total_booking_commission > 0 and not has_booking_invoice:
                # Générer UNE facture globale pour toutes les propriétés
                num_invoices = self.action_generate_booking_invoice()
                if num_invoices > 0:
                    results.append("Facture Booking.com globale créée")
                else:
                    results.append("Aucune facture Booking.com créée")
        ...
        # 2. FACTURES CONCIERGE PAR PROPRIÉTÉ (une par enregistrement)
        try:
            for record in monthly_records:
                if record.concierge_commission > 0 and not record.concierge_invoice_id:
                    try:
                        record.action_generate_both_concierge_invoices()
                        results.append(f"Factures concierge créées pour {self.month}/{self.year}")
                    except Exception as e:
                        results.append(f"Erreur facture concierge {self.month}/{self.year}: {str(e)}")

        except Exception as e:
            results.append(f"Erreur factures concierge: {str(e)}")
