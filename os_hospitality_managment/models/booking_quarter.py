...
class BookingQuarter(models.Model):
    ...
    tax_amount_month1 = fields.Monetary(string='Montant taxe mois 1', compute='_compute_tax_amounts',  
                                        currency_field='company_currency_id', store=True)
    tax_amount_month1_formatted = fields.Char(string='Montant mois 1 formaté', compute='_compute_tax_amounts_formatted',
                                              store=True)
    tax_amount_month2 = fields.Monetary(string='Montant taxe mois 2', compute='_compute_tax_amounts',  
                                        currency_field='company_currency_id', store=True)
    tax_amount_month2_formatted = fields.Char(string='Montant mois 2 formaté', compute='_compute_tax_amounts_formatted',
                                              store=True)
    tax_amount_month3 = fields.Monetary(string='Montant taxe mois 3', compute='_compute_tax_amounts',  
                                        currency_field='company_currency_id', store=True)
    tax_amount_month3_formatted = fields.Char(string='Montant mois 3 formaté', compute='_compute_tax_amounts_formatted',
                                              store=True)
    total_tax_amount = fields.Monetary(string='Total taxes à payer', compute='_compute_tax_amounts',
                                       currency_field='company_currency_id', store=True)
    total_tax_amount_formatted = fields.Char(string='Total taxes formaté', compute='_compute_tax_amounts_formatted',
                                             store=True)
    ...
    write_date_formatted = fields.Char(string='Date de modification', compute='_compute_write_date_formatted')
    ...
    def _compute_nights_data(self):
        ...
        for record in self:
            ...
            record.nights_month1 = nights_data['nights_month1'] + nights_data['exempt_nights_month1']
            record.nights_month2 = nights_data['nights_month2'] + nights_data['exempt_nights_month2']
            record.nights_month3 = nights_data['nights_month3'] + nights_data['exempt_nights_month3']
            record.total_nights = sum([nights_data[f'nights_month{i}'] for i in range(1, 4)] +
                                      [nights_data[f'exempt_nights_month{i}'] for i in range(1, 4)])
            ...
            record.taxable_nights_month1 = record.nights_month1 - record.exempt_nights_month1
            record.taxable_nights_month2 = record.nights_month2 - record.exempt_nights_month2
            record.taxable_nights_month3 = record.nights_month3 - record.exempt_nights_month3
            record.total_taxable_nights = record.total_nights - record.total_exempt_nights
    ...
    @api.depends('tax_amount_month1', 'tax_amount_month2', 'tax_amount_month3', 'total_tax_amount',
                 'company_currency_id')
    def _compute_tax_amounts_formatted(self):
        for record in self:
            currency = record.company_currency_id
            symbol = currency.symbol if currency else 'F'

            # Fonction de formatage avec espace comme séparateur de milliers
            def format_amount(amount):
                # Formater avec espace comme séparateur de milliers
                amount_str = f"{int(amount):,}".replace(',', ' ')
                return f"{amount_str} {symbol}"

            record.tax_amount_month1_formatted = format_amount(
                record.tax_amount_month1) if record.tax_amount_month1 else f"0 {symbol}"
            record.tax_amount_month2_formatted = format_amount(
                record.tax_amount_month2) if record.tax_amount_month2 else f"0 {symbol}"
            record.tax_amount_month3_formatted = format_amount(
                record.tax_amount_month3) if record.tax_amount_month3 else f"0 {symbol}"
            record.total_tax_amount_formatted = format_amount(
                record.total_tax_amount) if record.total_tax_amount else f"0 {symbol}"
    ...
    def _compute_tax_words(self):
        for record in self:
            if record.total_tax_amount:
                ...
                record.total_tax_words = "Zéro franc CFP"
