"""
Wizard OCR pour l'import d'ordonnances – v7.
Options :
...
class WizardOcrOrdonnance(models.TransientModel):
    ...
    # ── Stockage JSON des actes détectés (utilisé par action_appliquer) ───────
    # Contient la liste sérialisée des actes sous la forme :
    # [{"lettre_cle": "AMO", "coefficient": 10.0, "nb_seances": 5}, ...]
    # Alimenté aussi bien par le parser regex que par Claude.
    actes_json = fields.Char(string='Actes JSON (interne)', default='[]')
    ...
    def action_extraire(self):
        ...
        if self.use_claude_vision:
            ...
            if vision_vals.get('claude_status', '').startswith('✅'):
                ...
                vision_vals.setdefault(
                    'texte_extrait', _('(Image analysée directement par Claude vision)'))
        ...
        vals = {
            ...
            'actes_json': json.dumps(parsed['actes']),
            'etat': 'extrait',
        }

        # Appel Claude texte si demandé — ses résultats écrasent le parsing regex
    ...
    def _vals_from_claude_data(self, data):
        ...
        Stocke les actes à la fois dans actes_detectes (lisible) et actes_json
        (réutilisable par action_appliquer).
        """
        ...
        # ── Actes : stocker en JSON ET en texte lisible ───────────────────────
        actes = data.get('actes', [])
        # Normaliser les actes Claude au même format que le parser regex
        actes_norm = []
        for a in actes:
            try:
                actes_norm.append({
                    'lettre_cle': str(a.get('lettre_cle', '')).upper().strip(),
                    'coefficient': float(a.get('coefficient', 0)),
                    'nb_seances': int(a.get('nb_seances', 1)),
                })
            except (ValueError, TypeError):
                continue
        vals['actes_json'] = json.dumps(actes_norm)
        vals['actes_detectes'] = self._format_actes(actes_norm) or _('Aucun acte détecté.')
        return vals
    ...
    def action_appliquer(self):
        ...
        if self.prescripteur_nom:
            prescripteur = self.env['res.partner'].search(
                [('name', 'ilike', self.prescripteur_nom),
                 ('is_prescripteur', '=', True)], limit=1)
            if not prescripteur:
                cat = self.env.ref(
                    'os_auxiliaire_medical.partner_category_prescripteur',
                    raise_if_not_found=False)
                create_vals = {'name': self.prescripteur_nom}
                if cat:
                    create_vals['category_id'] = [(4, cat.id)]
                if self.prescripteur_code:
                    create_vals['vat'] = self.prescripteur_code
                prescripteur = self.env['res.partner'].create(create_vals)
            elif self.prescripteur_code and not prescripteur.vat:
                prescripteur.write({'vat': self.prescripteur_code})
            vals['prescripteur_id'] = prescripteur.id
        ...
        if self.patient_nom:
            patient = self.env['res.partner'].search(
                [('name', 'ilike', self.patient_nom),
                 ('is_patient_cps', '=', True)], limit=1)
            if not patient:
                cat = self.env.ref(
                    'os_auxiliaire_medical.partner_category_patient',
                    raise_if_not_found=False)
                nom_complet = ' '.join(filter(None, [
                    self.patient_nom, self.patient_prenom]))
                create_vals = {'name': nom_complet or self.patient_nom}
                if cat:
                    create_vals['category_id'] = [(4, cat.id)]
                patient = self.env['res.partner'].create(create_vals)
            vals['patient_id'] = patient.id

        if vals:
            ord_.write(vals)

        # ── Actes : source unique = actes_json ────────────────────────────────
        # actes_json est alimenté aussi bien par le parser regex que par Claude,
        # donc action_appliquer fonctionne identiquement dans les deux cas.
        try:
            actes = json.loads(self.actes_json or '[]')
        except (json.JSONDecodeError, TypeError):
            actes = []

        # Actes disponibles filtrés sur la profession du praticien
        profession = ''
        if ord_.praticien_id and hasattr(ord_.praticien_id, 'get_cps_profession_key'):
            profession = ord_.praticien_id.get_cps_profession_key() or ''

        acte_domain = [('active', '=', True)]
        if profession:
            acte_domain = [
                ('active', '=', True),
                '|',
        ...
        actes_disponibles = self.env['cps.acte.type'].search(acte_domain)

        for a in actes:
            lettre = (a.get('lettre_cle') or '').upper().strip()
            coeff  = float(a.get('coefficient') or 0)
            nb     = int(a.get('nb_seances') or 1)

            at = None

            # 1. Correspondance exacte : lettre_cle + coefficient
            candidats = actes_disponibles.filtered(
                lambda r, l=lettre, c=coeff:
                    (r.lettre_cle or '').upper() == l
                    and abs((r.coefficient_defaut or 0) - c) < 0.01
            )
            if candidats:
                at = candidats[0]

            # 2. Même lettre_cle, coefficient le plus proche
            if not at and lettre:
                candidats = actes_disponibles.filtered(
                    lambda r, l=lettre: (r.lettre_cle or '').upper() == l
                )
                if candidats:
                    at = min(
                        candidats,
                        key=lambda r, c=coeff: abs((r.coefficient_defaut or 0) - c)
                    )

            # 3. Correspondance floue sur les 2-3 premiers caractères
            if not at and len(lettre) >= 2:
                prefix = lettre[:3]
                candidats = actes_disponibles.filtered(
                    lambda r, p=prefix:
                        (r.lettre_cle or '').upper().startswith(p[:2])
                )
                if candidats:
                    at = min(
                        candidats,
            ...
            if not at:
                _logger.info(
                    'OCR wizard: aucun acte trouvé pour lettre=%s coeff=%s', lettre, coeff)
                continue

            # Éviter les doublons
            if ord_.ligne_ids.filtered(lambda l, a=at: l.acte_type_id == a):
                continue

            self.env['cps.ordonnance.ligne'].create({
                'ordonnance_id': ord_.id,
                'acte_type_id':  at.id,
                'lettre_cle':    at.lettre_cle,
                'coefficient':   at.coefficient_defaut,
                'tarif_unitaire': at.tarif_unitaire,
                'nb_seances':    nb,
            })
