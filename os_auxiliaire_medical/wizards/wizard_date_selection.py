"""
Wizard de saisie groupée de séances par sélection calendaire – v8.
...
Saisie de l'heure :
  • heure_debut sur le wizard : heure commune appliquée à toutes les séances générées
  • heure_acte sur chaque ligne : peut être ajustée individuellement avant d'appliquer
  • Propagé sur CpsActe.heure_acte, qui recalcule datetime_acte (UTC, fuseau Tahiti)
"""
import datetime
...
class WizardDateSelection(models.TransientModel):
    ...
    # ── Onchange heure → propage sur les lignes existantes ────────────────────
    ...
    def action_generer_dates(self):
        ...
        if has_date_fin and has_nb_seances:
            raise UserError(_('Indiquez soit une date de fin, soit un nombre de séances — pas les deux.'))
        ...
        if has_nb_seances and self.nb_seances > NB_SEANCES_MAX:
            raise UserError(_('Le nombre de séances ne peut pas dépasser %d (feuille FSA25).') % NB_SEANCES_MAX)
