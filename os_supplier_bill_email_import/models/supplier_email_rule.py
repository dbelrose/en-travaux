...
class SupplierEmailRule(models.Model):
    ...
    def _process_email(self, rule, msg_dict, sender, subject):
        ...
        # ── Extraction PDF ────────────────────────────────────────────────────
        pdf_list, image_list = rule._extract_pdf_attachments_from_msg_dict(msg_dict)
        pdf_bytes_list = (pdf_list, image_list)  # tuple passé à _get_parsing_text
        _logger.info(
            "_process_email [%s] : %d PDF + %d image(s) détecté(s).",
            rule.name, len(pdf_list), len(image_list),
        )

        # ── Tentative Factur-X ────────────────────────────────────────────────
        facturx_parsed = None
        facturx_lines = []
        if rule.facturx_contract_field and pdf_list:
        ...
        if rule.facturx_contract_field and pdf_list and not facturx_parsed:
            _logger.warning(
    ...
    def _get_parsing_text(self, msg_dict, pdf_bytes_list):
        """
        Détermine le texte à utiliser pour le parsing.

        pdf_bytes_list peut être :
          - une liste de bytes (rétrocompatibilité avec le wizard .eml)
          - un tuple (pdf_list, image_list) issu de
            _extract_pdf_attachments_from_msg_dict()

        Ordre d'extraction si use_pdf_attachment = True :
          1. PDFs (texte natif, puis OCR si scan détecté)
          2. Images (OCR direct via pytesseract + Pillow)
        """
        self.ensure_one()
        body_text = self._extract_text_from_msg_dict(msg_dict)
        pdf_text = ''

        if not self.use_pdf_attachment:
            return body_text, pdf_text

        # Normaliser pdf_bytes_list en (pdf_list, image_list)
        if isinstance(pdf_bytes_list, tuple) and len(pdf_bytes_list) == 2:
            pdf_list, image_list = pdf_bytes_list
            # Assurer le format [(bytes, mime), ...]
            if pdf_list and isinstance(pdf_list[0], bytes):
                pdf_list = [(b, 'application/pdf') for b in pdf_list]
            if image_list and isinstance(image_list[0], bytes):
                image_list = [(b, 'image/jpeg') for b in image_list]
        else:
            # Rétrocompatibilité : liste de bytes bruts
            pdf_list = [(b, 'application/pdf') for b in (pdf_bytes_list or [])]
            image_list = []
        ...
        for pdf_bytes, _ in pdf_list:
            try:
                ...
                if extracted.strip():
                    pdf_text = extracted
                    _logger.info(
                        "_get_parsing_text [%s] : texte extrait du PDF "
                        "(%d car.).", self.name, len(extracted.strip()),
                    )
                    break
            except Exception as exc:
                _logger.warning(
        ...
        # ── Extraction depuis les images si PDF vide ou absent ────────────
        if not pdf_text and image_list:
            if not OCR_AVAILABLE:
                _logger.warning(
                    "_get_parsing_text [%s] : %d image(s) jointe(s) mais OCR "
                    "non disponible. Installez : "
                    "pip install pdf2image pytesseract Pillow "
                    "&& apt install tesseract-ocr",
                    self.name, len(image_list),
                )
            else:
                for img_bytes, mime in image_list:
                    try:
                        extracted = extract_image_text(img_bytes, mimetype=mime)
                        if extracted.strip():
                            pdf_text = extracted
                            _logger.info(
                                "_get_parsing_text [%s] : texte extrait de "
                                "l'image OCR (%d car.).",
                                self.name, len(extracted.strip()),
                            )
                            break
                    except Exception as exc:
                        _logger.warning(
                            "_get_parsing_text [%s]: erreur OCR image — %s",
                            self.name, exc,
                        )
    ...
    def _extract_pdf_attachments_from_msg_dict(msg_dict):
        """
        Extrait les pièces jointes PDF ET images depuis le dict message Odoo.

        Retourne une liste de tuples (bytes, mimetype) pour permettre au
        code appelant de distinguer PDFs et images et d'appliquer le bon
        moteur d'extraction (pdfminer ou OCR direct).
        Pour rétrocompatibilité, extract_pdf_bytes() retourne uniquement
        les bytes comme avant — ce wrapper est utilisé par _get_parsing_text.
        """
        ...
        for att in attachments:
            ...
            is_image = (
                mime_lower in _IMAGE_MIMES
                or any(name_lower.endswith(ext) for ext in _IMAGE_EXTS)
            )

            if is_pdf:
                pdf_list.append((content, 'application/pdf'))
            elif is_image:
                image_list.append((content, mime_lower or 'image/jpeg'))

        return pdf_list, image_list

    @staticmethod
    def _extract_pdf_bytes_list(msg_dict):
        """Rétrocompatibilité : retourne uniquement les bytes PDF."""
        pdf_list, _ = SupplierEmailRule._extract_pdf_attachments_from_msg_dict(msg_dict)
        return [b for b, _ in pdf_list]
