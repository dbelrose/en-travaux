...
class SupplierEmailRule(models.Model):
    ...
    def _create_single_vendor_bill(self, parsed_data, product_tmpl, pdf_lines=None):
        ...
        # Diagnostic post-create : vérification de ce qu'Odoo a réellement
        # stocké — permet de détecter si l'ORM a ignoré ou modifié les lignes.
        self.env.cr.execute("""
            SELECT aml.id, aml.name, aml.price_unit, aml.quantity,
                   aml.price_subtotal, aa.code, aa.account_type
        ...
        for row in self.env.cr.fetchall():
            _logger.info(
                "  └ ligne %d : '%s' | compte=%s (%s) | "
                "price_unit=%.2f qty=%.2f subtotal=%.2f",
                row[0], (row[1] or '')[:40], row[5], row[6],
